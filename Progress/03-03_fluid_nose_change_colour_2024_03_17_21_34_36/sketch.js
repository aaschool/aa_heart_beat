...
function updateFluid() {
  if (isDrawing && poses.length > 0) {
    let firstPose = poses[0].pose;
    let firstNose = firstPose.nose;
    let mirroredX = width - firstNose.x;
    let col = floor(mirroredX / cellSize);
    let row = floor(firstNose.y / cellSize);
    let index = col + row * numCols;
    fluid[index] = 1;

    if (poses.length === 1) {
      fluidColor[0] = map(firstNose.x, 0, width, 0, 360);
      fluidColor[1] = map(firstNose.y, 0, height, 0, 100);
    }
  }
