...
let isDrawing = true;
let showCamera = true;
let noseColor, leftWristColor, rightWristColor;
...
function setup() {
  ...
  noseColor = color(255, 0, 0); // Red
  leftWristColor = color(0, 255, 0); // Green
  rightWristColor = color(0, 0, 255); // Blue
}
  ...
  if (isDrawing) {
    poses.forEach(pose => {
      ...
      if (nose.confidence > 0.5) {
        ...
        fluid[index] = { color: noseColor, strength: 1 };
      }
      ...
      if (leftWrist.confidence > 0.5) {
        ...
        let index = col + row * numCols;
        fluid[index] = { color: leftWristColor, strength: 1 };
      }
      ...
      if (rightWrist.confidence > 0.5) {
        ...
        let index = col + row * numCols;
        fluid[index] = { color: rightWristColor, strength: 1 };
      }
    });
  }
